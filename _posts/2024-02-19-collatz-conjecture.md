...
위 과정에서 $x_n$이 길이 $n$인 압축 사이클을 이룬다고 가정하자. 즉 $x_n$의 압축 수열 $x_n$, $x_{n-1}$, $\cdots$, $x_1$이 사이클이라고 가정하는 것이다. 이 경우 $x_n = x_1$로 표현할 수 있다. 

$x_n$이 $x_1$에 대한 일차식이므로 이것은 항상 해가 존재하며, 그 해는 계수인 $\{a_n\}$에 대한 식으로 표현된다. 

식을 유도하는 것은 어렵지 않으나 귀찮으므로 결과만 써보겠다. 보조 수열 $\{S_n\}$을 다음과 같이 정의하자. 

$$S_k = \begin{cases} \displaystyle \sum_{i=k+1}^{n}{a_i} & k < n \\
0 & k = n \end{cases}$$ 
...
$$q_n = \sum_{k=1}^{n-1} {\delta(d_k, p_k) - q_k} + \delta(d_n, p_n) \\
...
인지 확인하면 된다. 즉, 위 식이 참인 $n$에 대해 길이가 $n$인 홀수 사이클은 존재하지 않는다. 
