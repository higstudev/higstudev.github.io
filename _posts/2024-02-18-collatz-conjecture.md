...
이를 $\left\\{a_n\right\\}$을 타고간다,혹은  $\left\\{a_n\right\\}$을 거쳐간다고 표현하겠다. 
...
$$\begin{align}
x_{n+1} &= \frac{2^{S_0} x - \left( 2^{S_1} + 2^{S_2} \cdot 3 + \cdots + 2^{S_n} 3^{n-1} \right)}{3^n} \\
...
이므로 $q_n = \delta (d_n, p_n)$이면 $d_{n+1} = d_n$이다. $p_n$으로 올라간 것을 없앤 것이다. 
