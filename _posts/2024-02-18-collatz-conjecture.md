...
이게 무슨 얘기냐면, $\{a_n\}$을 타고 가는 사이클은 유리수 범위에서 항상 유일하게 존재하며 $\{a_n\}$에 대한 식으로 표현된다는 것이다. 만약 그 중에 자연수인 것이 존재하면 반례가 되는 것이다. 
...
나아가, $\{a_n\}$에 연속하여 나오는 1을 묶어보자. 혹은 $\{x_n\}$의 증감에 따라 묶는다고 생각해도 좋다. 말로 설명하기 어려우니 예를 들어보겠다. 
...
식이 매우 복잡해보이는데, $\mid N \mid = \sum_{k=1}^n q_k$이므로 
